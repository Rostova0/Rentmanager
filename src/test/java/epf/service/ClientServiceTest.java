...
public class ClientServiceTest {
    ...
    @Test
    public void testDeleteClient() throws DaoException, ServiceException {
        Client client = new Client();
        client.setId(1);

        when(clientDaoMock.delete(client)).thenReturn(1L);
        long deletedClientId = clientService.delete(client);

        verify(clientDaoMock).delete(client);
        assertEquals(client.getId(), deletedClientId);
    }
}
